...
import sys, csv
import numpy as np
from util import my_histogram_plot
...
def cnt_overall_rating_dis(inputfile, outputfile):
    ...
    my_histogram_plot(data, seg_num=20)
    raw_input()
...
def cnt_user_rating_dis(inputfile1, inputfile2, outputfile):
    '''
    ...
    for line in open(inputfile1):
        ...
        my_histogram_plot(score_cache, seg_num = 5)
        raw_input()
    ...
    for user_id in user_score:
        ...
        outputrow = map(row, lambda x:" ".join(x))
        outputstr = ",".join(outputrow)
        wfd.write("%s\n" % outputstr)
    wfd.close()

def cnt_item_rating_dis(inputfile1, inputfile2, outputfile):
    '''
    '''
...
if __name__ == "__main__":
    ...
    cnt_user_rating_dis(sys.argv[1], sys.argv[2], sys.argv[4])
    cnt_item_rating_dis(sys.argv[1], sys.argv[2], sys.argv[5])
