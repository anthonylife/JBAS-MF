...
def filter_term_by_legality(in_paths, out_paths):
    for i, in_path in enumerate(in_paths):
        ...
        for line in csv.reader(open(in_paths[i])):
            reviewid = line[0]
            aspects = map(lambda x:x.split(" ")[0], line[1:])
            modifiers = map(lambda x:x.split(" ")[1], line[1:])
            for j in range(len(aspects)):
                aspect = check_word(aspects[j])
                if aspect == None:
                    continue modifier = check_word(modifiers[i])
                if modifier == None:
                    continue
                review_phrase[reviewid].append([aspect, modifier])
...
def main():
    ...
    in_paths = [paths["filter_data_dir1"]+paths["phrase_review_file"],
    ...
    filter_term_by_legality(in_paths, out_paths)
