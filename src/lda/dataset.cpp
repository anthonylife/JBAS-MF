...
int Dataset::read_user_data(string user_file, int &nU, UserItem ** &users){
    ...
    char * pointer = NULL;
    UserItem * temp_user = NULL;
    char buff[BUFF_SIZE_LONG];
    for (int i = 0; i < nU; i++){
        temp_user = new UserItem();
        pointer = fgets(buff, BUFF_SIZE_LONG-1, fin);
        if (!pointer){
            printf("Unmatched number of text line of file %s!\n", user_file.c_str());
	        return RET_ERROR_STATUS;
        }
        nR += temp_user->parse_user_line(buff, TRAIN_DATA);
...
int Dataset::read_user_data(string user_file){
    ...
    for (int i = 0; i < tmp_nU; i++){
        ...
        nR += temp_user->parse_user_line(buff, TEST_DATA);
        memset(buff, 0, BUFF_SIZE_LONG);
    }
    fclose(fin);
...
int Dataset::read_item_data(string item_file, int &nI, UserItem ** &items){
    ...
    items = new UserItem*[nI+1];
    
    int st_review_idx;
    char buff[BUFF_SIZE_LONG];
    char * pointer = NULL;
    UserItem * temp_item = NULL;
    vector<Review*> sub_reviews;
    for (int i=0; i<nI; i++){
        temp_item = new UserItem();
        pointer = fgets(buff, BUFF_SIZE_LONG-1, fin);
        if (!pointer){
            printf("Unmatched number of text line of file %s!\n", item_file.c_str());
	        return RET_ERROR_STATUS;
        }
        sub_reviews = temp_item->parse_item_line(buff, TRAIN_DATA);
...
int Dataset::read_item_data(string item_file){
    ...
    for (int i=0; i<tmp_nI; i++){
        ...
        sub_reviews = temp_item->parse_item_line(buff, TEST_DATA);
        st_review_idx = reviewid2idx.size();
        for (unsigned int j=0; j<sub_reviews.size(); j++){
            reviewid2idx.insert(pair<int, int>(sub_reviews[j]->reviewid, st_review_idx+j));
            add_review(sub_reviews[j], st_review_idx+j);
...
int Dataset::map_reviewid_to_idx(const int nU, const int nI, UserItem ** users,
        UserItem ** items){
    unordered_map<int, int>::iterator iter;
    
    for (int i=1; i<=nU; i++){
        //users[i]->reviewidx_set = new int[users[i]->nreview];
        users[i]->reviewidx_set = zeros<vec>(users[i]->nreview);
    ...
    for (int i=1; i<=nI; i++){
        ...
        items[i]->reviewidx_set = zeros<vec>(items[i]->nreview);
...
int UserItem::parse_user_line(char * line, const string dataseg){
    vector<char *> line_segments = utils::split_str(line, ',');
    int tmp_nreview = line_segments.size()-1;
    
    if (dataseg == TRAIN_DATA){
        nreview = line_segments.size()-1;
        id = atoi(line_segments[0]);
...
vector<Review*> UserItem::parse_item_line(char * line, const string dataseg){
    vector<Review*> sub_reviews;
    vector<char *> line_segments = utils::split_str(line, ',');
    ...
    for (unsigned int i=1; i<line_segments.size(); i++){
        ...
        review->length = npair/2;
        review->z_ai = new int[review->length];
        review->z_au = new int[review->length];
        review->r_s  = new int[review->length];
        for (int j=0; j<npair/2; j++){
            review->headtermid[j] = atoi(subline_segments[2*j+1])-1;
            review->sentimentid[j] = atoi(subline_segments[2*j+2])-1;
        }
