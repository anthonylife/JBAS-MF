...
class Model{
    public:
        ...
        int * lda_nwsum_s;  // total number of sentiment words assigned to topic j
        int * lda_nwsum_a;  // total number of aspect words assigned to topic j
        int ** lda_nw_s;    // number of sentiment word i to topic j, V_s*K.
        int ** lda_nw_a;    // number of aspect word i to topic j, V_a*K.
        int ** lda_nd;      // number of word in doc i to topic j, RW*K
        mat lda_beta_s;     // sentiment word topic distribution
        mat lda_beta_a;     // aspect word topic distribution
        mat lda_alpha_r;    // review topic distribution
        ...
        int * lda_nwsum_s_t;  // total number of sentiment words assigned to topic j
        int * lda_nwsum_a_t;  // total number of aspect words assigned to topic j
        int ** lda_nw_s_t;    // number of sentiment word i to topic j, V_s*K.
        int ** lda_nw_a_t;    // number of aspect word i to topic j, V_a*K.
        int ** lda_nd_t;      // number of sentiment word in doc i to topic j, RW*K
        mat lda_beta_s_t;     // sentiment word topic distribution
        mat lda_beta_a_t;     // aspect word topic distribution
        mat lda_alpha_r_t;      // review topic distribution
        ...
        int lda_iters;          // iteration of gibbs sampling
        int lda_saveiter;       // start iteration for saving sampling
        int lda_savestep;       // interval step for saving smapling
        int lda_iters_t;
        int lda_saveiter_t;
        int lda_savestep_t;
        bool burntag;           // burning or not
        int burncnt;            // total burning count
        int burncnt_t;

        int lda_sampling_s(int reviewidx, int termidx);
        int lda_sampling_a(int reviewidx, int termidx);
        int lda_sampling_s_t(int reviewidx, int termidx);
        int lda_sampling_a_t(int reviewidx, int termidx);
        void compute_beta_s();
        void compute_beta_a();
        void compute_alpha();
        void compute_beta_s_t();
        void compute_beta_a_t();
        void compute_alpha_t();
        double compute_perp(string dataseg);
        double eval_review_perp(string dataseg, int reviewidx);
        //****************************************
        ...
        vec rr;                 // shuffle the all training index
        int sp_lnum;            // upper limit number of sampling
        // following are inter-results
        mat as_rating;
        mat mf_rating;
        mat inter_result;   
        int * tmp_cip_ra;
        int * tmp_cip_sum;
        int * tmp_cua_zu;
        //vec i_rr;               // shuffle each item training index
        //vec u_rr;               // shuffle each user training index
        ...
        double prediction(const string dataseg, int eval_method, bool save_tag);
        //void solve_regpara();           // solving the regression parameters
        
        int esti_aspect_sampling(int set_idx, int review_idx, int term_idx,
                int itemid);        // sampling aspect topic for item in est 
        int esti_rating_sampling(int set_idx, int review_idx, int term_idx,
                int itemid);        // sampling rating level in estimation
        int estu_aspect_sampling(int set_idx, int review_idx, int term_idx,
                int userid);        // sampling aspect topic for user in est
        ...
        vec get_item_reviewidx_vec(string dataseg, int itemid, int reviewidx);
        //vec get_item_vec(string dataseg, int userid);
        vec get_user_reviewidx_vec(string dataseg, int userid, int reviewidx);
...
double Model::rating_prediction(int uid, int iid, colvec aspect, colvec polarity){
    /*cout << aspect.t() << endl;
    cout << polarity.t() << endl;
    cout << user_factor.row(uid) << endl;
    cout << item_factor.row(iid) << endl;
    cout << user_bias(uid) << endl;
    cout << item_bias(iid) << endl;*/
    as_rating = lambda.t()*(aspect%polarity);
    mf_rating = user_factor.row(uid)*item_factor.row(iid).t();
