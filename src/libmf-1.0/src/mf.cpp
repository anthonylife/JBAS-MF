...
void output_trpara(char *model_path, Model *model){
    string data_dir[3] = {"/home/anthonylife/Doctor/Code/MyPaperCode/JBAS-MF/data/CellarTracker/train-test/","/home/anthonylife/Doctor/Code/MyPaperCode/JBAS-MF/data/Amazon-Movies/train-test/","/home/anthonylife/Doctor/Code/MyPaperCode/JBAS-MF/data/Amazon-Food/train-test/"};
    string user_factor_file = "libmf_user_factor.dat";
    string user_bias_file = "libmf_user_bias.dat";
    string item_factor_file = "libmf_item_factor.dat";
    string item_bias_file = "libmf_item_bias.dat";

    string user_factor_file_path;
    string user_bias_file_path;
    string item_factor_file_path;
    string item_bias_file_path;
    if (strstr(model_path, "cellar")){
        user_factor_file_path = data_dir[0]+user_factor_file;
        user_bias_file_path = data_dir[0]+user_bias_file;
        item_factor_file_path = data_dir[0]+item_factor_file;
        item_bias_file_path = data_dir[0]+item_bias_file;
    }else if (strstr(model_path, "movie")){
        user_factor_file_path = data_dir[1]+user_factor_file;
        user_bias_file_path = data_dir[1]+user_bias_file;
        item_factor_file_path = data_dir[1]+item_factor_file;
        item_bias_file_path = data_dir[1]+item_bias_file;
    }else if (strstr(model_path, "food")){
        user_factor_file_path = data_dir[2]+user_factor_file;
        user_bias_file_path = data_dir[2]+user_bias_file;
        item_factor_file_path = data_dir[2]+item_factor_file;
        item_bias_file_path = data_dir[2]+item_bias_file;
    }

    output_factor(model->P, model->nr_us, model->dim, user_factor_file_path);
    output_factor(model->Q, model->nr_is, model->dim, item_factor_file_path);
    output_bias(model->UB, model->nr_us, user_bias_file_path);
    output_bias(model->IB, model->nr_is, item_bias_file_path);
}

void output_factor(float *factor, int num, int dim, string file_path){
...
void output_bias(float *bias, int num, string file_path){
    FILE * fout = fopen(file_path.c_str(), "w");
    if (!fout){
        printf("Fail to create file %s!\n", file_path.c_str());
        exit(1);
